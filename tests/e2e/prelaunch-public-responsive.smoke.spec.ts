...
test.describe('pre-launch public UI responsive coverage', () => {
  for (const viewport of VIEWPORTS) {
    test(`client landing page is responsive at ${viewport.name}`, async ({ page }) => {
      ...
      await expect(page.locator('img[alt="Cleaner wiping a kitchen counter inside a residential home"]'))
        .toHaveAttribute('src', /home-cleaner-hero\.jpg/)
      await expect(page.locator('img[src*="hero-client.gif"]')).toHaveCount(0)
    ...
    test(`account access pages are responsive at ${viewport.name}`, async ({ page }) => {
      ...
      await expect(page.locator('img[alt="Cleaner performing a domestic cleaning task inside a residential home"]'))
        .toHaveAttribute('src', /account-access-cleaner\.jpg/)
      await expect(page.locator('img[src*="Become%20a%20Cleaner.png"]')).toHaveCount(0)
