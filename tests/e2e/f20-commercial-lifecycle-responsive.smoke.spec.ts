  ...
  test.describe('admin session', () => {
    ...
    test('E2E-RESP-04 dispute lifecycle and amendment audit stay readable at all viewports', async ({ page }) => {
      ...
      for (const viewport of VIEWPORTS) {
        ...
        await expect(bookingState.getByText('Cleaner location captured', { exact: true })).toBeVisible()
        await expect(bookingState.getByText('Yes', { exact: true })).toBeVisible()
        await expect(bookingState.getByText('Arrival verification', { exact: true })).toBeVisible()
        ...
        await expect(bookingState.getByText('GPS accuracy', { exact: true })).toBeVisible()
